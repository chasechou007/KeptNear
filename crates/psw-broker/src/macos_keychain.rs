...
impl DeviceKeyStore for MacOsDeviceKeyStore {
    ...
    fn create_new(&self, key: &DeviceRootKey) -> Result<(), DeviceKeyStoreError> {
        add_generic_password(Self::base_query(), key.expose()).map_err(|status| {
...
fn add_generic_password(query: Vec<(CFString, CFType)>, key: &[u8]) -> Result<(), i32> {
    let query = add_query(query, key);
...
fn add_query(mut query: Vec<(CFString, CFType)>, key: &[u8]) -> Vec<(CFString, CFType)> {
    ...
    unsafe {
        query.extend([
            ...
            (
                ...
                CFString::new(DEVICE_KEY_LABEL).into_CFType(),
            ),
            (
...
mod tests {
    ...
    fn create_query_is_device_only_and_load_query_requests_only_data() {
        ...
        let create = add_query(MacOsDeviceKeyStore::base_query(), &test_key);
